...
export default function NewLead() {
  ...
  const total = cart.reduce(
    (sum, item) => sum + item.product.prix_unitaire * item.quantity,
    0,
  );
  ...
  const handleSubmit = async () => {
    if (!client.name || !client.phone)
      return toast.error("Nom et téléphone client requis");
    if (cart.length === 0) return toast.error("Panier vide");
    if (balance < total)
      return toast.error(`Solde insuffisant: ${balance} DH < ${total} DH`);
    ...
    try {
      ...
      toast.success(
        `Commande ${res.data.n_lead} créée! Solde restant: ${res.data.solde_restant} DH`,
      );
      navigate("/leads");
    } catch (err: any) {
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="bg-blue-50 px-3 py-1 rounded-lg text-sm">
          Solde: <strong className="text-blue-600">{balance} DH</strong>
        </div>
      </div>
          ...
          {step === 2 && (
            <div className="card">
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  onClick={() => client.name && client.phone && setStep(3)}
              ...
              {cart.map((item) => (
                <div key={item.product.id} className="flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {item.product.prix_unitaire} DH/u
                    </p>
                  </div>
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-gray-800">
                      {(item.product.prix_unitaire * item.quantity).toFixed(2)}{" "}
                      DH
                    </p>
                    <button
