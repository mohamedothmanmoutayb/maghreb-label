...
import { Wallet, Plus, TrendingUp, TrendingDown, RefreshCw, Loader } from 'lucide-react';
...
export default function WalletPage() {
  ...
  const [rechargeForm, setRechargeForm] = useState({ montant: '', methode: 'virement', reference: '' });
  ...
  const txIcon = (type: string) => {
    if (type === 'debit') return <TrendingDown size={16} className="text-red-500" />;
    return <TrendingUp size={16} className="text-green-500" />;
  };

  const txColor = (type: string) => type === 'debit' ? 'text-red-600' : 'text-green-600';
  const txSign = (type: string) => type === 'debit' ? '-' : '+';

  const statusBadge = (s: string) => {
    const map: any = { pending: 'badge-pending', completed: 'badge-confirmed', rejected: 'badge-cancelled' };
    const labels: any = { pending: 'En attente', completed: 'Complété', rejected: 'Rejeté' };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Transactions */}
      <div className="card">
        <h3 className="font-semibold text-gray-700 mb-4">Historique des transactions</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {tx.reference_commande} • {new Date(tx.created_at).toLocaleDateString('fr-FR')}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className={`font-bold ${txColor(tx.type)}`}>
                    {txSign(tx.type)}{tx.montant} DH
                  </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b">
              <h2 className="text-xl font-bold text-gray-800">Demande de recharge</h2>
              <p className="text-sm text-gray-500 mt-1">Votre demande sera validée par un admin.</p>
            </div>
            <form onSubmit={handleRecharge} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={e => setRechargeForm(p => ({ ...p, montant: e.target.value }))}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => setRechargeForm(p => ({ ...p, methode: e.target.value }))}
                  className="input-field"
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setRechargeForm(p => ({ ...p, reference: e.target.value }))}
                  className="input-field"
